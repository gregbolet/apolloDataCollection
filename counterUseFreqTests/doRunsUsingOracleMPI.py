...
XTIME_FILE_PREFIX='oracle-depth7'
...
OUTPUT_XTIME_FILE = XTIME_FILE_PREFIX+'-ETE-XTimeData_VA'

if args.usePA:
	envvars['APOLLO_ENABLE_PERF_CNTRS'] = '1'
	OUTPUT_XTIME_FILE = XTIME_FILE_PREFIX+'-ETE-XTimeData_PA'
	pava = 'PA'
...
if args.makeTraces:
	...
	OUTPUT_XTIME_FILE += '-WITH_TRACES.csv'
else:
	envvars['APOLLO_TRACE_CSV'] = '0'
	OUTPUT_XTIME_FILE += '-NO_TRACES.csv'
...
def get_work_from_checkpoint():
	todo = [(x,y,a,b,w) for x in prognames for y in probSizes for a in countersToTry for b in cntrModes for w in range(NUM_TRIALS)]
	#todo.sort()
	print('todo', len(todo))
	df = pd.DataFrame(columns=['progname', 'probSize', 'papiCounter', 'cntrMode', 'trialnum', 'eteXtime'])
...
def doRunForProg(prog, probSize, papiCounter, cntrMode, trialnum, mystdout):
	...
	# quick modification to have folder contain depth7 in name
	apollo_trial_dir = progsuffix+'-'+probSize+'-'+papiCounter+'-'+cntrMode+'-usingOracle-'+XTIME_FILE_PREFIX+'-trial'+str(trialnum)+'-'+pava
	...
	envvars['APOLLO_POLICY_MODEL'] = 'DecisionTree,max_depth=7,load-dataset='+progsuffix+'-data/'+progsuffix+'-medprob-'+papiCounter+'-ORACLE-'+cntrMode+'-PA/Dataset-single-model.yaml'

	envvars['APOLLO_PERF_CNTRS_SAMPLE_EACH_ITER'] = '0'

	# for some reason this wasn't happening, putting it here to make sure it does...
	if args.makeTraces:
		envvars['APOLLO_TRACE_CSV'] = '1'
	else:
		envvars['APOLLO_TRACE_CSV'] = '0'
			...
			for worker in workerStates:
				...
				if workerState == DONE_WORK_TAG:
					...
					if isDone:
						...
						papiCounter = responseData[1][2]
						cntrMode    = responseData[1][3]
						trialnum    = responseData[1][4]
						eteXtime    = responseData[2]
						dataToAdd = {'progname': progname, 'probSize': probSize,
												 'papiCounter': papiCounter, 'cntrMode': cntrMode, 
