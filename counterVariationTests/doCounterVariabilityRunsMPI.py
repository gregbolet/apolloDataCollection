...
def get_work_from_checkpoint():
	todo = [(x,y,z,a,w) for x in prognames for y in probSizes for z in policies for a in countersToTry for w in range(NUM_TRIALS)]
	#todo.sort()
	print('todo', len(todo))
	df = pd.DataFrame(columns=['progname', 'probSize', 'policy', 'papiCounter', 'trialnum', 'eteXtime'])
...
def doRunForProg(prog, probSize, policy, papiCounter, trialnum, mystdout):
	...
	apollo_trial_dir = progsuffix+'-'+probSize+'-'+papiCounter+'-'+XTIME_FILE_PREFIX+'-trial'+str(trialnum)+'-'+pava
